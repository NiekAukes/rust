...
use rustc_middle::ty::{
    self, Const, FloatTy, IntTy, ParamEnv, PolyFnSig, Ty, TyCtxt, TyKind,
    TypeAndMut, UintTy,
};
...
use rustc_target::abi::{
    Abi, Align, FieldIdx, FieldsShape, LayoutS, Primitive, Scalar, Size, TagEncoding, VariantIdx, Variants
};
...
pub(crate) fn provide(providers: &mut Providers) {
    ...
    providers.kernel_fn_sig = |tcx, def_id| {
    ...
    providers.kernel_fn_abi_of_fn_ptr = |tcx, key| {
        let result = tcx.fn_abi_of_fn_ptr(key);
        Ok(readjust_fn_abi(tcx, result?))
    };
    providers.kernel_fn_abi_of_instance = |tcx, key| {
        let result = tcx.fn_abi_of_instance(key);
        Ok(readjust_fn_abi(tcx, result?))
    };
    ...
    fn clone_layout(layout: &LayoutS<FieldIdx, VariantIdx>) -> LayoutS<FieldIdx, VariantIdx> {
        let LayoutS {
            ref fields,
            ref variants,
            abi,
    ...
    providers.kernel_layout_of = |tcx, key| {
        let TyAndLayout { ty, mut layout } = tcx.layout_of(key)?;
...
impl<'tcx> ConvSpirvType<'tcx> for TyAndLayout<'tcx> {
    fn spirv_type(&self, mut span: Span, cx: &CodegenCx<'tcx>) -> Word {
        if let TyKind::Adt(adt, args) = *self.ty.kind() {
            ...
            // this part is commented out because it is used to handle intrinsic types,
