use std::{assert_matches::assert_matches, cell::{RefCell, RefMut}, hash::{Hash, Hasher}, iter, ops::Range};
use rustc_data_structures::{fx::{FxHashMap, FxHashSet}, sync::Lrc};
use rspirv::{dr, spirv::{self, Word}};
use rustc_codegen_ssa::traits::BackendTypes;
use rustc_middle::{bug, mir::{coverage::Op, interpret::ConstAllocation}, ty::TyCtxt};
use rustc_span::{source_map::SourceMap, FileName, FileNameDisplayPreference, SourceFile, Span, Symbol, DUMMY_SP};
use rustc_arena::DroplessArena;
use crate::{codegen_cx::CodegenCx, spirv_type::SpirvType, symbols::Symbols, target::SpirvTarget, target_feature::TargetFeature};
    ...
    pub fn const_fold_load(self, cx: &CodegenCx<'_>) -> Option<Self> {
        match self.kind {
            SpirvValueKind::Def(id) | SpirvValueKind::IllegalConst(id) => {
                let &entry = cx.module_builder.id_to_const.borrow().get(&id)?;
                match entry.val {
    ...
    pub fn def_with_span(self, cx: &CodegenCx<'_>, span: Span) -> Word {
        match self.kind {
            ...
            SpirvValueKind::IllegalConst(id) => {
                let entry = &cx.module_builder.id_to_const.borrow()[&id];
...
pub struct BuilderSpirv<'tcx> {
    ...
    enabled_capabilities: FxHashSet<spirv::Capability>,
...
impl<'tcx> BuilderSpirv<'tcx> {
    pub fn new(
        ...
        fn add_cap(
            ...
            enabled_capabilities: &mut FxHashSet<spirv::Capability>,
            cap: spirv::Capability,
        ) {
        ...
        fn add_ext(builder: &mut dr::Builder, enabled_extensions: &mut FxHashSet<Symbol>, ext: Symbol) {
        ...
        add_cap(&mut builder, &mut enabled_capabilities, spirv::Capability::Shader);
        ...
        add_cap(&mut builder, &mut enabled_capabilities, spirv::Capability::Linkage);

        builder.memory_model(spirv::AddressingModel::Logical, memory_model);
    ...
    pub fn finalize(self) -> dr::Module {
    ...
    pub fn builder(&self, cursor: BuilderCursor) -> RefMut<'_, dr::Builder> {
    ...
    pub(crate) fn def_constant_cx(
        ...
        let id = match val {
            ...
            SpirvConst::PtrTo { pointee } => {
                builder.variable(ty, None, spirv::StorageClass::Private, Some(pointee))
            }
        };
    ...
    pub fn set_global_initializer(&self, global: Word, initializer: Word) {
        ...
        inst.operands.push(dr::Operand::IdRef(initializer));
        module.types_global_values.push(inst);
    }

    pub fn select_block_by_id(&self, id: Word) -> BuilderCursor {
        fn block_matches(block: &dr::Block, id: Word) -> bool {
