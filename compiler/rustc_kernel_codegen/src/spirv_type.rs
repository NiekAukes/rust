...
impl SpirvType<'_> {
    ...
    pub fn def(self, def_span: Span, cx: &CodegenCx<'_>) -> Word {
        ...
        let result = match self {
            ...
            Self::Integer(width, signedness) => {
                ...
                match width {
                    8 if !cx.module_builder.has_capability(Capability::Int8) => cx.zombie_with_span(
                        result,
                        def_span,
                        &format!("`{u_or_i}8` without `OpCapability Int8`"),
                    ),
                    16 if !cx.module_builder.has_capability(Capability::Int16) => cx.zombie_with_span(
                        result,
                        def_span,
                        &format!("`{u_or_i}16` without `OpCapability Int16`"),
                    ),
                    64 if !cx.module_builder.has_capability(Capability::Int64) => cx.zombie_with_span(
                        result,
                        def_span,
            ...
            Self::Float(width) => {
                ...
                match width {
                    64 if !cx.module_builder.has_capability(Capability::Float64) => cx.zombie_with_span(
                        result,
                        def_span,
    ...
    pub fn sizeof(&self, cx: &CodegenCx<'_>) -> Option<Size> {
        let result = match *self {
            ...
            Self::Array { element, count } => {
                cx.lookup_type(element).sizeof(cx)? * cx.module_builder.lookup_const_u64(count).unwrap()
            }
    ...
    fn display(&self, stack: &mut Vec<Word>, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self.ty {
            ...
            SpirvType::Array { element, count } => {
                let len = self.cx.module_builder.lookup_const_u64(count);
