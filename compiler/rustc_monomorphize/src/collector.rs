    ...
    match starting_item.node {
        MonoItem::Static(def_id) => {
            ...
            if mode == CollectionMode::UsedItems {
                ...
                debug_assert!(should_codegen_locally(tcx, instance, is_in_kernel));
                ...
                if let Ok(alloc) = tcx.eval_static_initializer(def_id) {
                    for &prov in alloc.inner().provenance().ptrs().values() {
                        collect_alloc(tcx, prov.alloc_id(), is_in_kernel, &mut used_items);
                    }
                }
                ...
                for (op, op_sp) in asm.operands {
                    match op {
                        ...
                        hir::InlineAsmOperand::SymFn { anon_const } => {
                            ...
                            visit_fn_use(tcx, fn_ty, false, *op_sp, is_in_kernel, &mut used_items);
                        }
                        hir::InlineAsmOperand::SymStatic { path: _, def_id } => {
                            let instance = Instance::mono(tcx, *def_id);
                            if should_codegen_locally(tcx, instance, is_in_kernel) {
...
impl<'a, 'tcx> MirVisitor<'tcx> for MirUsedCollector<'a, 'tcx> {
    fn visit_rvalue(&mut self, rvalue: &mir::Rvalue<'tcx>, location: Location) {
        ...
        match *rvalue {
            ...
            mir::Rvalue::Cast(
                ...
                visit_fn_use(self.tcx, fn_ty, false, span, self.is_in_kernel, self.used_items);
            }
            mir::Rvalue::Cast(
                ...
                if let ty::Closure(def_id, args) = *source_ty.kind() {
                    ...
                    if should_codegen_locally(self.tcx, instance, self.is_in_kernel) {
    ...
    fn visit_constant(&mut self, constant: &mir::ConstOperand<'tcx>, location: Location) {
        ...
        collect_const_value(self.tcx, val, self.is_in_kernel, self.used_items);
    }
    ...
    fn visit_terminator(&mut self, terminator: &mir::Terminator<'tcx>, location: Location) {
        ...
        match terminator.kind {
            ...
            mir::TerminatorKind::Drop { ref place, .. } => {
                ...
                visit_drop_use(self.tcx, ty, true, self.is_in_kernel, source, self.used_items);
            }
...
fn visit_drop_use<'tcx>(
    ...
    visit_instance_use(tcx, instance, is_direct_call, is_in_kernel, source, output);
}
...
fn collect_alloc<'tcx>(
    ...
    match tcx.global_alloc(alloc_id) {
        ...
        GlobalAlloc::Function(fn_instance) => {
            if should_codegen_locally(tcx, fn_instance, is_in_kernel) {
...
fn collect_items_of_instance<'tcx>(
    ...
    for const_op in &body.required_consts {
        if let Some(val) = collector.eval_constant(const_op) {
            collect_const_value(tcx, val, is_in_kernel, mentioned_items);
        }
    }
    ...
    for item in &body.mentioned_items {
        if !collector.used_mentioned_items.contains(&item.node) {
            ...
            visit_mentioned_item(tcx, &item_mono, item.span, is_in_kernel, mentioned_items);
        }
    }
}
...
fn visit_mentioned_item<'tcx>(
    ...
    match *item {
        ...
        MentionedItem::Drop(ty) => {
            visit_drop_use(tcx, ty, /*is_direct_call*/ true, is_in_kernel, span, output);
        }
        ...
        match self.tcx.def_kind(id.owner_id) {
            DefKind::Enum | DefKind::Struct | DefKind::Union => {
                    ...
                    && self.tcx.generics_of(id.owner_id).count() == 0
                {
                    ...
                    visit_drop_use(self.tcx, ty, true, false, DUMMY_SP, self.output);
                }
            }
            DefKind::GlobalAsm => {
                debug!(
            ...
            DefKind::Const => {
                ...
                if let Ok(val) = self.tcx.const_eval_poly(id.owner_id.to_def_id()) {
                    collect_const_value(self.tcx, val, false, self.output);
                }
            }
            DefKind::Impl { .. } => {
                if self.strategy == MonoItemCollectionStrategy::Eager {
...
fn create_mono_items_for_default_impls<'tcx>(
    ...
    for method in tcx.provided_trait_methods(trait_ref.def_id) {
        ...
        if mono_item.node.is_instantiable(tcx) && should_codegen_locally(tcx, instance, false) {
